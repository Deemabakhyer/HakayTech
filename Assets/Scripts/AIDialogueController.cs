...
public class AIDialogueController : MonoBehaviour
{
    ...
    [Range(1, 15)] public int aiAppearState = 1;
    [Range(1, 15)] public int boyShockState = 2;
    public float boyShockDelay = 0.15f;
    ...
    public void ShowState(int index)
    {
        ...
        if (index == aiAppearState && boyController != null)
        {
            CancelInvoke(nameof(TriggerBoyShock));
            Invoke(nameof(TriggerBoyShock), boyShockDelay);
        }
    }

    void TriggerBoyShock()
    {
        if (boyController != null)
            boyController.ShowState(boyShockState);
    }
